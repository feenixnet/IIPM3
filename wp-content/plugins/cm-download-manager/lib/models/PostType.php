...
abstract class CMDM_PostType {
    ...
    private static $postTypesToRegister = [];
    private static $taxonomiesToRegister = [];
    protected $ID;
    protected $post;
    protected $postMeta = [];
     ...
     * @return void
     */
    protected static function registerPostType($postType, $singular = '', $plural = '', $menu='', $args = []) {
    ...
    private static function addRegisterPostTypesHooks() {
        ...
        if (!$registered) {
            ...
            add_action('template_redirect', [__CLASS__, 'contextFixer']);
     ...
     * @return void
     */
    protected static function registerTaxonomy($taxonomy, $postTypes, $singular = '', $plural = '', $args = []) {
        ...
        self::$taxonomiesToRegister[$taxonomy] = [
    ...
    private static function addRegisterTaxonomiesHooks() {
        ...
        if (!$registered) {
            $registered = TRUE;
            add_action('init', [__CLASS__, 'registerTaxonomies'], 0);
        }
    }
...
//         $this->loadPostMeta();
    ...
    protected function registerUpdateHooks() {
        add_action('save_post', [$this, 'postUpdated'], 1000, 2);
        add_action('updated_post_meta', [$this, 'postMetaUpdated'], 1000, 4);
        add_action('added_post_meta', [$this, 'postMetaUpdated'], 1000, 4);
    }
    ...
    protected function unregisterUpdateHooks() {
        remove_action('save_post', [$this, 'postUpdated'], 1000, 2);
        remove_action('updated_post_meta', [$this, 'postMetaUpdated'], 1000, 4);
        remove_action('added_post_meta', [$this, 'postMetaUpdated'], 1000, 4);
    }
    ...
    public function addPostMeta($meta = [], $unique = FALSE) {
        foreach ($meta as $key => $value) {
    ...
    public static function findByMeta($post_type, $meta = []) {
        global $wpdb;
