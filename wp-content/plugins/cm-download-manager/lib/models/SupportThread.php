...
class CMDM_SupportThread {
    ...
    public static function init() {
        add_action('generate_rewrite_rules', array(__CLASS__, 'fixThreadSlugs'), 999);
    ...
    public static function getThreadsForDownload($id, $page = 1, $perPage = 10) {
        ...
        $sql = $wpdb->prepare("SELECT SQL_CALC_FOUND_ROWS c.comment_ID FROM {$wpdb->comments} c JOIN {$wpdb->commentmeta} cm ON c.comment_parent=0 AND c.comment_ID=cm.comment_id AND cm.meta_key='_thread_updated' AND c.comment_post_id=%d AND c.comment_approved=1 ORDER BY cm.meta_value*1 DESC LIMIT %d, %d",
            $id, $offset, $perPage);
    ...
    public static function renderDaysAgo($date, $gmt = false) {
        ...
        if ( $seconds_ago < 0) {
                return __('some time ago', 'cm-download-manager');
        } else {
            if ( $seconds_ago < 60 ) {
    ...
    public static function getThread($thread_id, $withComments = true) {
        ...
        if ( empty($lastPoster) ) {
			$lastPoster = $comment->comment_author;
		}
        $retVal = array(
            'thread_id' => $thread_id,
            ...
            'listeners' => get_comment_meta($thread_id, '_thread_listeners',
                true),
            'resolved' => (bool) get_comment_meta($thread_id,
                '_thread_resolved', true),
    ...
    public static function fixThreadSlugs($wp_rewrite) {
		$slug = CMDM_GroupDownloadPage::$rewriteSlug . '/';
        $wp_rewrite->rules = array(
            $slug . '(.+?)/topic/add/?$' => $wp_rewrite->index . '?' . CMDM_GroupDownloadPage::POST_TYPE . '=' . $wp_rewrite->preg_index(1) . '&CMDM-comment-action=add',
            $slug . '(.+?)/topic/([0-9]{1,})/?$' => $wp_rewrite->index . '?' . CMDM_GroupDownloadPage::POST_TYPE . '=' . $wp_rewrite->preg_index(1) . '&CMDM-comment-action=show&CMDM-comment-id=' . $wp_rewrite->preg_index(2),
            $slug . '(.+?)/topic/([0-9]{1,})/add/?$' => $wp_rewrite->index . '?' . CMDM_GroupDownloadPage::POST_TYPE . '=' . $wp_rewrite->preg_index(1) . '&CMDM-comment-action=add&CMDM-parent-id=' . $wp_rewrite->preg_index(2),
            $slug . '(.+?)/topic/page/([0-9]{1,})/?$' => $wp_rewrite->index . '?' . CMDM_GroupDownloadPage::POST_TYPE . '=' . $wp_rewrite->preg_index(1) . '&CMDM-comment-action=list&CMDM-comment-page=' . $wp_rewrite->preg_index(2),
            ) + $wp_rewrite->rules;
    }
    ...
    protected static function _notifyOnFollow($thread, $lastCommentId)
    {
        ...
        if (!empty($listeners)) {
            $message = CMDM_Settings::getOption(CMDM_Settings::OPTION_THREAD_NOTIFICATION);
            $title = CMDM_Settings::getOption(CMDM_Settings::OPTION_THREAD_NOTIFICATION_TITLE);
            ...
            foreach($listeners as $user_id) {
                if ($user = get_userdata($user_id) AND !empty($user->user_email)) {
            		$headers[] = ' Bcc: '. $user->user_email;
            	}
            }
            if (!empty($headers)) wp_mail(null, $title, $message, $headers);
//             foreach ($listeners as $user_id) {
...
//             }
    ...
    protected static function _notifyOnNewThread($thread_id, $post_id)
    {
        ...
        if (!empty($author)) {
            $message = CMDM_Settings::getOption(CMDM_Settings::OPTION_NEW_THREAD_NOTIFICATION);
            $title = CMDM_Settings::getOption(CMDM_Settings::OPTION_NEW_THREAD_NOTIFICATION_TITLE);
            $postTitle = $post->post_title;
            $threadTitle = $thread['title'];
    ...
    public static function updateThreadMetadata($thread_id, $lastCommentId, $lastAuthorId, $notify = false, $resolved = false)
    {
        ...
        update_comment_meta($thread_id, '_thread_updated',
            current_time('timestamp', true));
        update_comment_meta($thread_id, '_thread_posts', $counter);
        if ($resolved)
                update_comment_meta($thread_id, '_thread_resolved', $resolved);
        update_comment_meta($thread_id, '_thread_last_poster',
            $user->display_name);
        self::_notifyOnFollow($thread, $lastCommentId);
        if ($notify) {
            $listeners = (array) get_comment_meta($thread_id,
                    '_thread_listeners', true);
    ...
    public static function addCommentToThread($post_id, $thread_id, $content,
        $author_id, $notify = false, $resolved = false)
    {
        $user = get_userdata($author_id);

        // find HTML between <pre></pre> and repelace < >
        $tagOne = "<pre>";
        $tagTwo = "</pre>";
        $string = " " . $content;
        $ini = strpos($string, $tagOne);
        if ($ini != 0) {
            $ini += strlen($tagOne);
            $len = strpos($string, $tagTwo, $ini) - $ini;
            $parsed = htmlentities(substr($string, $ini, $len));
            $startTagPos = strrpos($content, $tagOne);
            $endTagPos = strrpos($content, $tagTwo);
            $tagLength = $endTagPos - $startTagPos;
            $content = substr_replace($content, $parsed, $startTagPos,
                $tagLength);
        }
        ...
        if (empty($content))
                $errors[] = __('Content cannot be empty',
                'cm-download-manager');
        if (!empty($errors)) {
        ...
        self::updateThreadMetadata($thread_id, $comment_id, $author_id, $notify,
            $resolved);
        return $comment_id;
    }

    public static function addThread($id, $title, $content, $author_id,
        $notify = false)
    {
        $user = get_userdata($author_id);
        ...
        if (empty($title))
                $errors[] = __('Title cannot be empty',
                'cm-download-manager');
        if (empty($content))
                $errors[] = __('Content cannot be empty',
                'cm-download-manager');
        ...
        $comment_id = wp_insert_comment($data);
        if ($comment_id) {
            update_comment_meta($comment_id, '_thread_updated',
                current_time('timestamp', true));
            update_comment_meta($comment_id, '_thread_posts', 1);
            update_comment_meta($comment_id, '_thread_title', $title);
            update_comment_meta($comment_id, '_thread_last_poster',
                $user->display_name);
            if ($notify) {
                $listeners = (array) get_comment_meta($comment_id,
                        '_thread_listeners', true);
                $listeners[] = $author_id;
                $listeners = array_unique($listeners);
