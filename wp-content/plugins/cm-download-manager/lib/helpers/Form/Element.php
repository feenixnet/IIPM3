...
class CMDM_Form_Element {
    ...
    public function getLabel() {
        return apply_filters('cmdm_form_element_label', $this->_label, $this->getId());
    }
    ...
    public function getDescription() {
        return apply_filters('cmdm_form_element_description', $this->_description, $this->getId());
    }
    ...
    public function isValid($value, $context = null, $showError = false, $obj = null) {
        ...
        if ($this->isRequired() && empty($value) && (is_array($value) OR strlen($value) == 0)) {
            if ($showError) $this->addError($this->_requiredError);
            return false;
        } else if (!empty($value) || (is_scalar($value) && strlen($value) > 0)) {
            $valid = true;
            foreach ($this->getValidators() as $validator) {
    ...
    public function setTrClass($className) {
        $this->_tr_class = $className;
        return $this;
    }
    ...
    public function __toString() {
        ...
        $html .= '<tr class="' . esc_attr($this->getId()) . ' ' . $this->_tr_class . '"><td>' .
                    $label . '</td><td>' . $this->render() . '</td></tr>';
