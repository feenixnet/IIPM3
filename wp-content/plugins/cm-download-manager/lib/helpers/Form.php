...
class CMDM_Form
{
    ...
    public function addElement(CMDM_Form_Element $elem, $position = 99)
    {
    ...
    {
        $pos = array();
        foreach($this->_elements as $key => $row) $pos[$key] = $row['pos'];
    ...
    public function getErrors()
    {
        ...
        {
            if( count($elem['elem']->getErrors()) > 0 ) $errors = array_merge($errors, $elem['elem']->getErrors());
        }
        return $errors;
    }
    ...
    public function getErrorElements()
    {
        ...
        foreach($this->_elements as $elem)
        {
            if( count($elem['elem']->getErrors()) > 0 ) $errors[] = $elem['elem']->getId();
        }
    ...
    public function __toString()
    {
        ...
        if( !empty($enctype) )
        {
            $enctype = ' enctype="' . esc_attr($enctype) . '"';
        }
        
        $html .= '<form method="' . esc_attr($this->getMethod()) . '" action="' . esc_attr($this->getAction()) . '"' . $enctype . ' class="CMDM-form" autocomplete="off"><table border="0">';
